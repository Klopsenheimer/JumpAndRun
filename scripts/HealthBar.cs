...
public partial class HealthBar : Control
{
	...
	private void UpdateHealthBar()
	{
		if (healthBar != null)
		{
			...
			if (healthPercentage > 0.6f) styleBox.BgColor = Colors.Green;
			else if (healthPercentage > 0.3f) styleBox.BgColor = Colors.Yellow;
			else styleBox.BgColor = Colors.Red;
			healthBar.AddThemeStyleboxOverride("fill", styleBox);
		}
		if (healthLabel != null) healthLabel.Text = $"{currentHealth}/{maxHealth}";
	}
	...
	public void TakeDamage(int damage)
	{
		...
		if (currentHealth <= 0) EmitSignal(SignalName.HealthDepleted);
	}
